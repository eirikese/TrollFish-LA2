...
function getVideoIngestConcurrency() {
  ...
  // Desktop edition: native FS reads, no browser tab throttling — parallelize
  // up to roughly half the cores (GPMF extraction is I/O + parse bound).
  if (globalThis.window?.trollfishDesktop?.isDesktop) {
    if (Number.isFinite(cores) && cores > 0) return Math.max(2, Math.min(6, Math.floor(cores / 2)));
    return 3;
  }
...
function probeVideoDuration(file) {
  return new Promise((resolve) => {
    // Desktop edition: path-backed file streams via protocol (no Blob URL).
    const nativeUrl = file._tfNativePath
      ? 'trollfish-file://local/' + encodeURIComponent(file._tfNativePath) : null;
    const url = nativeUrl || URL.createObjectURL(file);
    const video = document.createElement('video');
    video.preload = 'metadata';
    let resolved = false;
    const done = (val) => { if (resolved) return; resolved = true; if (!nativeUrl) { try { URL.revokeObjectURL(url); } catch {} } resolve(val); };
