...
async function openTimelineMediaOverlay(item) {
  ...
  // Desktop edition: path-backed files stream via protocol (no Blob URL).
  _timelineMediaOverlayUrl = file._tfNativePath
    ? 'trollfish-file://local/' + encodeURIComponent(file._tfNativePath)
...
// Desktop edition: a File-like object backed by an absolute disk path.
// slice()/arrayBuffer()/text() read on demand via the native bridge, so
// multi-GB GoPro videos are never loaded whole into memory.
function makeNativeFile(p) {
  const bridge = window.trollfishDesktop;
  const sliceRange = (start, end) => {
    const s = Math.max(0, Math.min(start ?? 0, p.size));
    const e = Math.max(s, Math.min(end ?? p.size, p.size));
    return {
      size: e - s,
      arrayBuffer: () => bridge.readFileRange(p.path, s, e),
  ...
  return {
    name: p.name,
    size: p.size,
    type: /\.(mp4|mov|m4v)$/i.test(p.name) ? 'video/mp4'
        : /\.csv$/i.test(p.name) ? 'text/csv' : '',
    lastModified: p.lastModified,
    _tfNativePath: p.path,
    slice: (start, end) => sliceRange(start, end),
    arrayBuffer: () => bridge.readFile(p.path),
    async text() { return new TextDecoder().decode(new Uint8Array(await bridge.readFile(p.path))); },
  };
}
